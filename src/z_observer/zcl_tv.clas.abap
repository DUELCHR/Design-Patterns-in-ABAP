...
CLASS zcl_tv IMPLEMENTATION.


  METHOD zif_display~display.
   mo_out->write( |TV: Temperatur { mo_wheather_station->get_temperature( ) } and Pressure { mo_wheather_station->get_pressure( ) }| ).
...
METHOD zif_display~display_using_events.
  mo_wheather_station ?= sender.
      mo_out->write(
        |TV: Temperatur { mo_wheather_station->get_temperature( ) } and Pressure { mo_wheather_station->get_pressure( ) }| ).
  ENDMETHOD.
